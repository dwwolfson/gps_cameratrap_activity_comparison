...
##########################################################################################################################################
data <- read.csv("BIR_SummerGPS_Females.csv") 
head(data)
...
data2 <- read.csv("BIR_SummerGPS_Males.csv") 
head(data2)
...
MaleFemale_Summer_overlap<-overlapEst(randTimesdf$timeRad, randTimesdf2$timeRad, type = "Dhat4") 
MaleFemale_Summer_overlap

##########################################################################################################################################
tiff("BIR_MaleFemale_Summer_Overlap.tiff", width = 6, height = 6, units = 'in', res = 300)
overlapPlot(randTimesdf$timeRad, randTimesdf2$timeRad, main="Summer Wild Pig Activity Curves Florida", linetype = c(2, 1), 
linecol = c("black", "black"),linewidth = c(2, 2), ylab="Activity", olapcol = "lightgrey",adjust=1, cex.lab=1.5, cex.axis=1.5, family = "serif")
legend("top", c("Females (n=23)", "Males (n=20)"), "Overlap=0.XX"), lty=c(1,2,0), col = c(1,1,0), bty="n")
dev.off()
