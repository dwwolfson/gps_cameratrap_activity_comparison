...
##########################################################################################################################################
data <- read.csv("BIR_FallGPS_Females.csv") 
head(data)
...
data2 <- read.csv("BIR_FallGPS_Males.csv") 
head(data2)
...
MaleFemale_Fall_overlap<-overlapEst(randTimesdf$timeRad, randTimesdf2$timeRad, type = "Dhat4") 
MaleFemale_Fall_overlap

##########################################################################################################################################
tiff("BIR_MaleFemale_Fall_Overlap.tiff", width = 6, height = 6, units = 'in', res = 300)
overlapPlot(randTimesdf$timeRad, randTimesdf2$timeRad, main="Fall Wild Pig Activity Curves Florida", linetype = c(2, 1), 
linecol = c("black", "black"),linewidth = c(2, 2), ylab="Activity", olapcol = "lightgrey",adjust=1, cex.lab=1.5, cex.axis=1.5, family = "serif")
legend("top", c("Females (n=18)", "Males (n=7)"), "Overlap=0.XX"), lty=c(1,2,0), col = c(1,1,0), bty="n")
dev.off()
